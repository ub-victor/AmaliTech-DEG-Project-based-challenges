export default function NodeCard({ node, isSelected, onClick }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Input port (top center) – visual indicator */}
        <div className="absolute -top-1.5 left-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-gray-400 border border-white z-10" />
        ...
        {!isEnd && node.options.map((opt, idx) => (
          ...
          >
            <span>{opt.label}</span>
            {/* Output port (right edge, per option) */}
            <div className="absolute -right-1.5 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-gray-400 border border-white z-10" />
          </div>
        ))}
